...
$sql_er = 'SELECT id, er_name, SUM(zhi_hangzhou) FROM test002 GROUP BY er_name ORDER BY id ASC';
...
$rrr['series'][2]['radius'][0] = "85%";
$rrr['series'][2]['radius'][1] = "95%";
...
//shuffle($result_er);
for ($i = 0; $i < count($result_er); $i++){
    if((float)$result_er[$i][2] > 0){
        $tmp = array();
        $tmp['name'] = $result_er[$i][1];
        $tmp['value'] = $result_er[$i][2];
        array_push($rrr['series'][0]['data'], $tmp);
    }
}

for ($i = 0; $i < count($rrr['series'][0]['data']); $i++){
    $sql_san = "SELECT id, san_name, SUM(zhi_hangzhou) FROM test002 WHERE er_name ='".$rrr['series'][0]['data'][$i]['name']."' GROUP BY san_name ORDER BY id ASC";
...
for ($i = 0; $i < count($rrr['series'][1]['data']); $i++){
    $sql_si = "SELECT id, si_name, SUM(zhi_hangzhou) FROM test002 WHERE san_name ='".$rrr['series'][1]['data'][$i]['name']."' GROUP BY si_name ORDER BY id ASC";
    $result_si = mysqli_query($connect, $sql_si)->fetch_all();
    for ($j = 0; $j < count($result_si); $j++){
        if((float)$result_si[$j][2] > 0){
            $tmp = array();
            $tmp['name'] = $result_si[$j]['1'];
            $tmp['value'] = $result_si[$j]['2'];
