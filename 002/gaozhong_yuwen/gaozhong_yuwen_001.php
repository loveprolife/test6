...
$sql_yi = 'SELECT A.id, A.yi_name, SUM(zhi) FROM (SELECT * FROM gao_zhong_yuwen_001 WHERE is_show = 1 ORDER BY id ASC) AS A GROUP BY A.yi_name ORDER BY A.id ASC';
...
for ($i = 0; $i < count($rrr['series'][0]['data']); $i++){
    $sql_er = "SELECT id, er_name, SUM(zhi) FROM gao_zhong_yuwen_001 WHERE is_show = 1 AND yi_name ='".$rrr['series'][0]['data'][$i]['name']."' GROUP BY er_name ORDER BY id ASC";
...
for ($i = 0; $i < count($rrr['series'][1]['data']); $i++){
    $sql_san = "SELECT id, san_name, SUM(zhi) FROM gao_zhong_yuwen_001 WHERE is_show = 1 AND er_name ='".$rrr['series'][1]['data'][$i]['name']."' GROUP BY san_name ORDER BY id ASC";
